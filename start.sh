...
INTERNAL_GRPC_PORT="7000" cargo run -p namenode >/dev/null 2>&1 < /dev/null &

sleep 5

# Run DataNode 1 (client port 8000, internal 3000)
echo "Starting DataNode 1..."
INTERNAL_GRPC_PORT="8000" INTERNAL_TCP_PORT="3000" NODE_ID="datanode_1" cargo run -p datanode >/dev/null 2>&1 < /dev/null&

# Run DataNode 2 (client port 9000, internal 4000)
echo "Starting DataNode 2..."
INTERNAL_GRPC_PORT="9000" INTERNAL_TCP_PORT="4000" NODE_ID="datanode_2"  cargo run -p datanode >/dev/null 2>&1 < /dev/null&
